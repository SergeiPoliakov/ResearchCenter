...
public final class constants {
	 ...
	 * Database queries
	 */
	public static final String SELECT_PRIORITY_TABLE = 
			"SELECT "
			+ " main_fo.FIN_OBJECT_ID as object_ID, "
			+ " main_fo.USER_ID,main_fo.OBJECT_NAME,"
			+ " (SELECT select_par.VALUE1 "
				...
				+ " WHERE select_pja.ATTRIBUTE_NAME='Коэфицент приоритета' "
			...
			+ " WHERE select_pja.ATTRIBUTE_NAME='Минимальный % от расхода' "
				+ " AND main_fo.FIN_OBJECT_ID = select_fo.FIN_OBJECT_ID "
				+ " AND main_fo.FIN_OBJECT_ID = select_par.FIN_OBJECT_ID) as min_percent,"
			+ " (SELECT select_par.VALUE1 "
			+ " FROM FIN_OBJECTS select_fo "
				+ " INNER JOIN PJ_ATTRIBUTES select_pja "
					+ " ON select_fo.FIN_OBJECT_TYPE_ID = select_pja.FIN_OBJECT_TYPE_ID "
			...
			+ " ) as final_date, --время до конца расчетн. периода может будет влиять"
			+ " ( "
			+ " SELECT "
